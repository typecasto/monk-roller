...
fn main() -> anyhow::Result<()> {
    ...
    let mut hits = vec![(
        d1 + ATTACK + map + if buffed { 1 } else { 0 } + if attack_type == "fist" { 1 } else { 0 },
        d1,
    )];
    if fob {
    ...
    for crit in results.iter().cloned().filter_map(|x| x) {
        ...
        let (short, long, all) = dmg
            ...
            .map(|(ty, mut dice)| {
                ...
                (
                    dice
                        // .inspect(|x| println!("{:?}", x))
                        .fold(first, |acc, x| acc + " + " + &x.to_string())
                        + ty,
                    sum,
                    ty,
                )
        ...
        if crit {
            ...
            cc::log::info(
                "crit! Target is stupefied 1, and frightened 2 if they were already stupified.",
            )?;
            ...
            {
                cc::log::info(format!(
                    "crit! Target is {} 1 until the end of your next turn.",
                    match attack_type {
                        "laser" => "dazzled",
                        "fist" => "slowed",
                        _ => "huh whuh",
    ...
    if results.len() != 0 && fob {
        ...
        if stun != 0 {
            cc::log::info(format!(
                "Target is stunned {stun} until the end of your next turn."
            ))?;
        }
    }
